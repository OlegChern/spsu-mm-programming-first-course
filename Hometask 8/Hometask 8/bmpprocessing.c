...
{
	FILE *bmpIn;
	errno_t r = fopen_s(&bmpIn, fileName, "rb");
	if (r != 0)
	{
...
{
	FILE *bmpOut;
	errno_t r = fopen_s(&bmpOut, fileName, "wb");
	if (r != 0)
	{
		...
		for (y = 0; y < width; y++)
		{
			...
			for (i = -nHalf; i <= nHalf; i++)
				for (j = -nHalf; j <= nHalf; j++)
				{
					...
					resPixel.rgbtRed += (unsigned char)(triple.rgbtRed * matrix[(i + nHalf)*n + j + nHalf]);
					resPixel.rgbtGreen += (unsigned char)(triple.rgbtGreen * matrix[(i + nHalf)*n + j + nHalf]);
					resPixel.rgbtBlue += (unsigned char)(triple.rgbtBlue * matrix[(i + nHalf)*n + j + nHalf]);
				}
