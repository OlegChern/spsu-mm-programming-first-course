...
_Filter Gaussian3x3Filter;
_Filter Gaussian5x5Filter;
_Filter SobelXFilter;
_Filter SobelYFilter;
_Filter ScharrXFilter;
_Filter ScharrYFilter;

int initTable(int ***arr, int h, int w)
{
    if ((*arr = static_cast<int **>(malloc(sizeof(**arr) * h))) == nullptr)
        return 0;

    for (int i = 0; i < h; ++i) {
        if (((*arr)[i] = static_cast<int *>(malloc(sizeof(***arr) * w))) == nullptr)
            return 0;
    }
...
int initFilter(_Filter *filter, int n, const int *table, int alpha)
{
    filter->n = n;
...
{
    byte *tmp;
    size_t tmpLen = static_cast<size_t>(map->h * map->w);
    if ((tmp = static_cast<byte *>(malloc(tmpLen))) == nullptr)
        exit(1);
...
int filterMap(BGRMap *src, BGRMap *des, int y, int x, _Filter *filter)
{
    for (int c = 0; c < 3; ++c)
    {
        long long sum = 0;
...
int gaussianMap(BGRMap *src, BGRMap *des, int y, int x, int n)
{
    ...
    _Filter filter = n == 3 ? Gaussian3x3Filter : Gaussian5x5Filter;
